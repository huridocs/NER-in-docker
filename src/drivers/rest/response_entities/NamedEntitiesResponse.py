...
class NamedEntitiesResponse(BaseModel):
    ...
    def from_groups(named_entity_groups: list[NamedEntityGroup]) -> "NamedEntitiesResponse":
        ...
        groups_response = NamedEntitiesResponse._create_groups_response(entities_response)
    ...
    def _sort_groups(groups_response: list[GroupResponse]) -> list[GroupResponse]:
        ...
        return sorted(groups_response, key=group_min_index)

    @staticmethod
    def _create_groups_response(entities_response: list[NamedEntityResponse]) -> list[GroupResponse]:
        ...
        for entity in sorted_entities_by_score:
            ...
            if group_name not in groups_response:
                groups_response[group_name] = GroupResponse.from_entity(entities_response, entity)
                continue
