...
class TestNamedEntityGroup(TestCase):
    def test_exact_reference_match(self):
        group = NamedEntityGroup(name="Test reference", type=NamedEntityType.REFERENCE)
        matches = group.get_references_in_text("Test reference")
        self.assertTrue(len(matches) > 0)
        self.assertEqual((0, 14), matches[0])

    def test_no_reference_for_non_REFERENCE_type(self):
    ...
    def test_reference_found_when_name_in_text(self):
        group = NamedEntityGroup(name="Section 1.1", type=NamedEntityType.REFERENCE)
    ...
    def test_no_reference_when_name_not_in_text(self):
        group = NamedEntityGroup(name="Chapter 5", type=NamedEntityType.REFERENCE)
    ...
    def test_no_reference_when_name_is_substring_of_other(self):
        group = NamedEntityGroup(name="Section 1", type=NamedEntityType.REFERENCE)
        text = "Please see Section 1.1 for details."
        matches = group.get_references_in_text(text)
        self.assertEqual(matches, [])

    def test_no_reference_with_empty_text(self):
        group = NamedEntityGroup(name="Appendix A", type=NamedEntityType.REFERENCE)
    ...
    def test_reference_with_colon_title_and_partial_reference(self):
        group = NamedEntityGroup(name="Document 3: Advanced Research", type=NamedEntityType.REFERENCE)
    ...
    def test_no_reference_for_numbered_title_with_only_partial_word(self):
        group = NamedEntityGroup(name="4. Results Interpretation", type=NamedEntityType.REFERENCE)
        text = (
    ...
    def test_multiple_references_to_same_title(self):
        group = NamedEntityGroup(name="Section 2.1", type=NamedEntityType.REFERENCE)
