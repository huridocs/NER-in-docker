...
class TestSQLiteEntitiesStoreRepository(TestCase):
    def setUp(self):
        entity_1 = NamedEntity(
            text="María Diaz", type=NamedEntityType.PERSON, segment=Segment.from_text("", "test_source_1")
        )
        entity_2 = NamedEntity(
            text="María Diaz Perez", type=NamedEntityType.PERSON, segment=Segment.from_text("", "test_source_1")
        )
    ...
    def test_save_entities(self):
        entity_1 = NamedEntity(
            text="HURIDOCS ORG", type=NamedEntityType.ORGANIZATION, segment=Segment.from_text("", "test_source_2")
        )
        entity_2 = NamedEntity(
            text="HURIDOCS", type=NamedEntityType.ORGANIZATION, segment=Segment.from_text("", "test_source_2")
        )
